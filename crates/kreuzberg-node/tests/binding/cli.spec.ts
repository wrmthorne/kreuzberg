...
describe("CLI", () => {
	...
	describe("CLI binary detection", () => {
		it("should find kreuzberg-cli 4.3.1 when available", () => {
		...
		it("should handle missing kreuzberg-cli 4.3.1", () => {
			const mockWhichSync = vi.mocked(which.sync);
	...
	describe("CLI argument passing", () => {
		it("should pass arguments to kreuzberg-cli 4.3.1", () => {
			const mockWhichSync = vi.mocked(which.sync);
			const mockSpawnSync = vi.mocked(spawnSync);
		...
		it("should handle --version flag", () => {
			...
			mockSpawnSync.mockReturnValue({
				...
				pid: 12345,
				output: [],
				stdout: Buffer.from("kreuzberg-cli 4.3.1"),
				stderr: Buffer.from(""),
				signal: null,
			});
